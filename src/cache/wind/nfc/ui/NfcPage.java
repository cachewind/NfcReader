...
import cache.wind.nfc.SPEC.EVENT;
import cache.wind.nfc.nfc.bean.Card;
import cache.wind.nfc.nfc.reader.ReaderListener;
	...
	private Intent buildResult(Card card) {
		...
		if (card != null && !card.hasReadingException()) {
			if (card.isUnknownCard()) {
				ret.putExtra(RET, NfcReaderApplication
			...
			ret.putExtra(RET,
					NfcReaderApplication.getStringResource(R.string.info_nfc_error));
		}
